...
```
Phase 0a 专家角色绑定    玄提案 -> 素质疑 -> 玄素收敛 -> 你确认
                         -> 写入 .roundtable/roles/{xuan,su}.expert.md
Phase 0  想法圆桌        玄/素带着专家能力打磨方向 -> 方向陈述         (你 + 玄 + 素)
                         ╔═ Gate 0：方向锁定 ═╗
...
专家角色绑定是每个 idea 的常驻专业能力层：玄/素仍然是固定流程角色，但会先根据 idea 绑定适合的
专业能力。panel 不是常驻 agent：Phase 1 强制、Phase 2 按高风险/复杂度触发；它是一次性补盲评审，
不碰 relay 路由，也不增加 tmux pane。
...
就会把对应的操作契约（`protocol.md` + `xuan.md`/`su.md` + 当前 idea 的 `*.expert.md`）发给它。你只需把原始想法发给
**左侧（玄）** pane——之后中继就接管了。
...
`.roundtable/roles/{xuan,su}.expert.md`、`docs/design/roundtable/{architecture,flow,requirements,decisions}.md`、
`.roundtable/channel.md` 和自己的 inbox，因此中途重启一个项目时会从上次交接处续上（全新项目里这些产物是空模板，
于是它会先做 Phase 0a 专家绑定）。如果你只是从一个仍在运行的会话 detach 了，则**无需 kickoff**——直接
`tmux attach` 回去即可。
...
两边重读 expert 绑定、docs 设计产物、`.roundtable/channel.md` 和自己的 inbox。做到一半的项目会从上次交接处续上；
全新项目则会先要求完成 Phase 0a 专家绑定。这是 `stop`、崩溃、机器重启、或 CLI 更新之后的常规路径——产物就是
...
| `.roundtable/roles/xuan.expert.md`、`su.expert.md` | 当前 idea 的玄/素专业能力绑定 | 否（运行时态） |
